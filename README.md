...
ProofLog currently has an initial Rust CLI with local config path handling, SQLite schema initialization, owner-only file permissions on Unix-like systems, and first-time `doctor` readiness output. The binary builds and exposes the planned MVP command surface:
