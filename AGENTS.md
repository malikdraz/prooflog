...
- `prooflog ingest --codex` currently discovers local `.jsonl` files, records file metadata, and stores non-empty raw JSONL lines with parse errors when malformed.
