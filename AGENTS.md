...
- `prooflog proof --since <REF>` currently detects repo root, branch, HEAD, merge base, dirty status, changed files, diff stats, docs-only status, risky changed paths, and relevant/ambiguous Codex sessions from local storage. Final proof report generation is still planned.
